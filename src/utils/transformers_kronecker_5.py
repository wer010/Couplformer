...
class Attention(Module):
    ...
    def forward(self, x):
        ...
        #x_h=(128,1,256,64,8);(b,s,c,hw,w)
        x_w = self.fc1(x_h)
        #x_w=(128,1,256,64,64);(b,s,c,hw,hw)
        x_c = x_w.permute(0, 1, 3, 4, 2)
        #x_c=(128,1,64,64,256);(b,s,hw,hw,c)
        x_c = self.fc2(x_c).permute(0, 1, 4, 2, 3)
        #x_c=(128,1,1,64,64);(b,s,c,hw,hw)
        Attn = x_c.reshape(B, self.num_heads, N, N)
        x = torch.matmul(Attn, V)
        # x_attn = torch.matmul(x_h, x)
        # x_attn = torch.matmul(x_attn, x_w)

        # x_fuse = torch.cat([x_c, x_h, x_w], dim=2)
        # x = self.fc3(x_fuse.permute(0, 1, 3, 4, 2))
